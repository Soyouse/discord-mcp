/**
 * Discord MCP — transport HTTP (StreamableHTTP) pour le service 24/7 distant.
 *
 ...
 *   - Auth Bearer OBLIGATOIRE : le serveur REFUSE de démarrer sans DISCORD_MCP_HTTP_TOKEN.
...
const TOKEN = process.env.DISCORD_MCP_HTTP_TOKEN;
const HOST = process.env.DISCORD_MCP_HTTP_HOST || "127.0.0.1";
const PORT = Number(process.env.DISCORD_MCP_HTTP_PORT || 8788);
...
app.listen(PORT, HOST, () => {
  process.stderr.write(`Discord MCP HTTP sur http://${HOST}:${PORT}/mcp (Bearer requis)\n`);
});
