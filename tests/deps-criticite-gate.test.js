...
// ⚠️⚠️ CE REPO N'A AUCUN MOTEUR AUJOURD'HUI (serveur = transport, front = affichage VU par un humain) — ET C'EST EXACTEMENT LE CAS DANGEREUX. Un gate dont le
...
describe("criticité des dépendances", () => {
  test("toute dépendance est CLASSÉE dans deps-criticite.json (non classée = ROUGE, jamais un oubli silencieux)", () => {
    ...
    expect(deps.length).toBeGreaterThanOrEqual(40);
