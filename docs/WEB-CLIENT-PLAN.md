...
- **P5 — Front.** SPA React+Vite : **page de login (OAuth Discord)** + garde de route (JWT), sidebar serveurs/DM, liste users DMables, fil + composer, live WS. nginx statique + bloc compose. **Découpé en sous-phases prouvables :**
  ...
  - **P5b — Layout réel + composants** (rail bots, liste convs, fil virtualisé `@tanstack/react-virtual`, composer, panneau détails). Composants purs testés.
  - **P5c — Données** : client API typé + hooks react-query (guilds/channels/dmables/history) ; **mock = MSW** (standard) pour rendre l'UI avec données réalistes AVANT P2b/API live. Réconciliation à brancher.
  - **P5d — Temps réel** : `socket.io-client`, abonnement room salon, réconciliation optimiste (l'écho P4 revient par socket → dédupe par message_id).
