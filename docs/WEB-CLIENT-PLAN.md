...
- **P2 — API web squelette. 🔶 (cœur fait)** `web/` : `config.js` (env-schema, refuse de booter si invalide) · `read-service.js` (pur, réutilise query.js, projection publique) · `auth.js` (guard JWT + `claimsToPrincipal`) · `tenant.js` (couture) · `build-app.js` (Fastify : helmet/cors/rate-limit/jwt + routes, testé via `inject`) · `routes-read.js` (GET guilds/channels/history/search/dmables) · `server.js` (entrypoint). 207 tests, mutation 88.27%.
- **P2b — Login with Discord (OAuth2 → JWT). ✅ LIVE & PROUVÉ (2026-06-07, PR #11/#13).** `oauth.js` (helpers PURS : authorize URL, allowlist FAIL-CLOSED, mapping user) · `oauth-service.js` (login/refresh/logout, deps injectées, mutation 88.65%) · `refresh-store.js`(pur+mémoire : hashToken sha256, isUsable)/`refresh-store-pg.js`(I/O, exclu mutation) — refresh tokens HASHÉS, rotation anti-rejeu · `oauth-io.js` (échange code + /users/@me, fetch injecté, client_secret jamais côté navigateur) · `routes-auth.js` (login/callback state-CSRF, refresh=cookie HttpOnly+Secure+SameSite=Lax, access JWT en RAM front). Front : `useSession` (refresh mount + avant expiry) + garde de route (anon→/login) + `LoginPage` actif + déconnexion ; MSW mocke /refresh en dev/e2e, vrai backend en prod. Allowlist `.env OAUTH_ALLOWED_USER_IDS` (Vegeta+Soyouse+waikoz). **Prouvé** : login Discord réel → cockpit authentifié VegeBlox, vrais salons + historique Postgres. **Pièges scellés** : `.gitattributes eol=lf` (git archive CRLF cassait build.sh) ; `apiFetch` content-type-si-body (Fastify 400 body vide, masqué par MSW → tester le contrat en live) ; compose `discord-web` doit lister les vars OAuth.
...
- **P5 — Front.** SPA React+Vite : **page de login (OAuth Discord)** + garde de route (JWT), sidebar serveurs/DM, liste users DMables, fil + composer, live WS. nginx statique + bloc compose. **Découpé en sous-phases prouvables :**
  ...
  - **P5e — ⌘K + polish. ✅** `MessageRow` : rendu **markdown** (`react-markdown`+`remark-gfm`, `skipHtml` anti-XSS, JAMAIS de parsing maison) + temps relatif (`date-fns` locale fr) + état `pending` (optimiste, opacité + "envoi…"). `CommandPalette` (`cmdk`, contrôlée) + `useCommandPalette` (raccourci ⌘K/Ctrl+K) = saut clavier vers une conversation, icônes `lucide-react`. 58 tests front (markdown, code inline, pending, palette filtre/sélection, hook ⌘K). ⚠️ Polyfills jsdom dans test-setup (`ResizeObserver`/`scrollIntoView` requis par cmdk). `react-hook-form` REPORTÉ (aucun formulaire réel avant P2b login). Screenshots `front-cmdk.png` + markdown rendu. ⚠️ Bundle grossit (markdown/cmdk) → code-splitting = optim P6.
...
- **P6 — Déploiement + durcissement. 🔶 (deploy fait ✅)** **DÉPLOYÉ & PROUVÉ LIVE 2026-06-06** (PR #6) : conteneurs `discord-web` (API Fastify, MÊME image Node, bind 127.0.0.1:8080 interne) + `discord-web-front` (nginx, image séparée multi-stage Vite→nginx:1.27-alpine, bind IP Tailscale 100.64.0.1:**8790**, sert le SPA + reverse-proxy /api+/socket.io même origine → zéro CORS). HTTPS = `tailscale serve --https=8451` (cert LE *.ts.net) — preuve depuis un autre nœud : `/api/health`→{"ok":true} verify=0, SPA `<title>Cockpit Discord</title>`, `/api/guilds`→401 (JWT requis, attendu pré-P2b), fallback SPA `/cockpit`→200. Code-splitting Vite (manualChunks react/query/markdown/realtime) ✅. Build des 2 images prouvé en CI (`docker build` front-door). Secret `WEB_JWT_SECRET` en `.env` VPS. ⚠️ Ports VPS : 8788=MCP/8789=publer-mcp/8790=web ; serve 8449/8450/8451. ⚠️ `/opt/discord-mcp` = sync par TAR (pas git) ; build = `nohup` OBLIGATOIRE (SIGHUP sinon). **RESTE P6 : permissions fines, rate-limit UI, observabilité (prom-client/Sentry), visual regression** (Playwright `toHaveScreenshot`, rendu Docker épinglé sinon flaky Windows↔CI).

> **Filet anti-régression front (ajouté 2026-06-06, hors phase — exigence « zéro régression ») :** la mutation testing N'EST PAS la pratique front Big Tech (outil de logique pure/backend). Le vrai filet = couches : unit/composants (vitest+@testing-library) + intégration réseau (MSW) + **E2E parcours** (Playwright assertions, gate CI ; webServer = `preview` du build `--mode e2e` AVEC MSW, PAS `vite dev` — le watcher s'orphelinait sur Windows : SIGTERM ignoré → fuites accumulées ; garantie dure de non-fuite = l'éphémère CI) + **mutation sur la LOGIQUE PURE** (Stryker `front/stryker.conf.json` : reconcile/http/endpoints, break=80, JAMAIS les composants JSX) + visual regression (P6) + TypeScript (écarté, choix JS). Tout dans le job CI `front`. 64 tests + 6 E2E + mutation 92.31%.
