...
- **P3 — Temps réel. 🔶 (P3a fait : bus + mapping purs)** `relay/events.js` (`toEvent` pur, 100% mutation, EVENT_CHANNEL partagé) · `relay/publish-pg.js` (PG NOTIFY, I/O seam → Redis/NATS) · `listener.js` publie après écriture (publish injecté, défaut noop) · `web/realtime.js` (`parseNotification`+`eventToEmit` purs, room `channel:<id>`). Test de contrat de bridge (même canal des 2 côtés). **RESTE P3b : serveur Socket.IO attaché à Fastify (auth JWT à la connexion, join room par salon) + boucle PG LISTEN → `io.to(room).emit`. Multi-nœuds = `@socket.io/redis-adapter` (seam).**
