...
let sessionBot = null; // défaut posé par discord_switch_bot (mono-session only)
...
export function resolveBotId(requested) {
  const id = requested || sessionBot || defaultBot;
  if (!bots[id]) {
...
export async function listBots() {
  await loadSecrets();
  return { bots: Object.keys(bots), default: defaultBot, session: sessionBot };
}

/** Pose le défaut de session (utilisé par discord_switch_bot). Valide l'existence. */
export async function setSessionBot(id) {
  await loadSecrets();
  if (!bots[id]) {
    const known = Object.keys(bots).join(", ");
    throw new Error(`Bot inconnu : ${id} (disponibles : ${known})`);
